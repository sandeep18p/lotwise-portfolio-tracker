...
console.log('=== WORKER DATABASE CONFIGURATION DEBUG ===');
...
const testConnection = async () => {
  try {
    console.log(`🔄 Worker attempting database connection (attempt ${connectionAttempts + 1}/${maxAttempts})...`);
    ...
    console.log('✅ Worker database query test successful:', result.rows[0]);
    ...
    connectionAttempts++;
    console.error(`❌ Worker database connection test failed (attempt ${connectionAttempts}/${maxAttempts}):`);
    ...
    if (connectionAttempts < maxAttempts) {
      ...
      console.log(`⏳ Worker retrying connection in ${delay/1000} seconds...`);
      setTimeout(testConnection, delay);
    } else {
      console.error('❌ Worker max connection attempts reached. Worker will continue without database.');
      console.error('This may cause trade processing to fail.');
      isConnected = false;
    }
    return false;
  }
...
pool.on('connect', (client) => {
  console.log('✅ New client connected to PostgreSQL database (Worker)');
...
pool.on('acquire', (client) => {
  console.log('🔄 Worker client acquired from pool');
});

pool.on('remove', (client) => {
  console.log('🗑️ Worker client removed from pool');
});

pool.on('error', (err, client) => {
  console.error('❌ Unexpected error on idle client (Worker):', err);
  console.error('Error details:', {
    code: err.code,
    message: err.message,
    errno: err.errno,
...
const gracefulShutdown = async () => {
  console.log('🔄 Worker gracefully shutting down database pool...');
  try {
    await pool.end();
    console.log('✅ Worker database pool closed successfully');
  } catch (err) {
    console.error('❌ Error closing worker database pool:', err);
  }
};
...
module.exports = {
  ...
  query: async (text, params) => {
    ...
    try {
      console.log('🔄 Worker executing query:', text.substring(0, 100) + (text.length > 100 ? '...' : ''));
      const result = await pool.query(text, params);
      const duration = Date.now() - start;
      console.log(`✅ Worker query executed successfully in ${duration}ms`);
      return result;
    } catch (err) {
      const duration = Date.now() - start;
      console.error(`❌ Worker query failed after ${duration}ms:`, err.message);
      ...
      if (err.code === 'ENETUNREACH' || err.code === 'ECONNREFUSED') {
        isConnected = false;
        console.error('🔄 Network error detected, worker connection status updated');
      }
